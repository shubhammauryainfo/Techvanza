...
const Feedback = () => {
  ...
  const columns = [
    ...
    { key: "createdAt", label: "Date", render: (row) => format(new Date(row.createdAt), 'Pp') },
    {
      key: "action",
      label: "Action",
      width:"4px",
  ...
  const deleteFeedback = async (_id) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/forms/${_id}`, {
        method: "DELETE",
        headers: {
          "auth-key": apiKey || "",
        },
